...
class CarRegistrPage extends StatefulWidget {
  const CarRegistrPage({super.key});

  @override
  State<CarRegistrPage> createState() => _CarRegistrPageState();
}

class _CarRegistrPageState extends State<CarRegistrPage> {
  ...
  void _handleOnPressedSubmit() async {
    if (plate.text.isEmpty) {
      COSnackBar.show(context, title: "Plate can't empty");
      return;
    }

    if (_selectCarTypes == null) {
      COSnackBar.show(context, title: "Car type can't empty");
      return;
    }
